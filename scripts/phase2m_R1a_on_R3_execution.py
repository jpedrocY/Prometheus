"""Phase 2m R1a-on-R3 first-execution runner.

Runs the Phase 2j R1a candidate (Volatility-percentile setup) on the
locked R3 exit baseline (Fixed-R take-profit + unconditional time-stop)
against the locked Phase 2e v002 datasets. Includes the H0 control and
the R3-only locked control on the same engine version for the §10.3
H0-anchor comparison.
...
Variants:
    ...
    R1a+R3 — R3 exit machinery + R1a setup-validity predicate
            (setup_predicate_kind=VOLATILITY_PERCENTILE,
             setup_percentile_threshold=25,
             setup_percentile_lookback=200).

Per the Phase 2m operator-approved brief + Phase 2k Gate 1 §11.B + Phase
2j memo §C:

  - Research-only. No live trading, no exchange adapter, no keys.
  - Same v002 datasets as Phase 2e/2g/2l. No data downloads.
  - One variant per invocation.
  - H0 + R3 controls re-run on the same engine version. The Phase 2e
    baseline run dir stays untouched.
  - R1a sub-parameters are committed singularly per Phase 2j memo §C.6.
  - Mark-price stop-trigger semantics preserved as default; TRADE_PRICE
    is a sensitivity diagnostic per GAP-20260424-032.

Usage::

    uv run python scripts/phase2m_R1a_on_R3_execution.py --variant H0     --window R
    uv run python scripts/phase2m_R1a_on_R3_execution.py --variant R3     --window R
    uv run python scripts/phase2m_R1a_on_R3_execution.py --variant R1a+R3 --window R
    uv run python scripts/phase2m_R1a_on_R3_execution.py --variant R1a+R3 --window V
    uv run python scripts/phase2m_R1a_on_R3_execution.py --variant R1a+R3 --window R \\
        --slippage HIGH
    uv run python scripts/phase2m_R1a_on_R3_execution.py --variant R1a+R3 --window R \\
...
Output directory::
    ...
    data/derived/backtests/phase-2m-r1a/<variant>/<window>/<experiment>/<run_id>/
        <SYMBOL>/
...
# Phase 2m carry-forward set: H0 control, R3 locked control, R1a-on-R3 candidate.
# Phase 2j-committed sub-parameters singularly; no sweeps, no extra candidates.
...
def _experiment_label(variant_id: str) -> str:
    """File-system-friendly experiment-name segment."""
    return variant_id.lower().replace("+", "_plus_")


def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Phase 2m R1a-on-R3 first-execution runner")
...
def main() -> int:
    ...
    experiment_name = f"phase-2m-r1a-{_experiment_label(variant_id)}-{window_id.lower()}"
    if experiment_parts:
    ...
    _banner(
        f"Phase 2m R1a-on-R3 | variant={variant_id} window={window_id}\n"
    ...
    accepted_limits = [
        f"Phase 2m R1a-on-R3 first-execution variant={variant_id} window={window_id} "
        f"slippage={slippage_bucket.value} stop_trigger={stop_trigger.value}.",
        "R1a spec is structural redesign per Phase 2j memo §C (setup_predicate_kind="
        "VOLATILITY_PERCENTILE; X=25 percentile threshold; N=200 lookback). "
        "Sub-parameter values are committed singularly with no sweeps.",
        "R3 exit baseline locked per Phase 2j memo §D (exit_kind=FIXED_R_TIME_STOP; "
        "R-target=2.0 R; time-stop bars=8). Sub-parameter values unchanged from Phase 2l.",
        ...
        "GAP-20260424-031 (EMA slope definition): inherited; R1a does not touch the bias rule.",
        "GAP-20260424-032 (mark-price stop-trigger sensitivity): runs emit "
        "stop_trigger_source in the manifest; R1a-on-R3 promotion includes a TRADE_PRICE pass.",
        "GAP-20260424-033 (R3 unconditional time-stop): inherited from R3; semantically "
        "distinct from H0 STAGNATION (which has an MFE gate).",
        "Phase 2g wave-1 REJECT ALL preserved as historical evidence; no comparison-baseline "
        "shifting. Phase 2l R3 PROMOTE preserved; R3-only is the locked exit baseline.",
    ...
    _banner(f"Phase 2m R1a-on-R3 run complete. run_id={run_id}")
