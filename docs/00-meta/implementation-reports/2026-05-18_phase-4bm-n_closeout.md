...
**Date:** 2026-05-18.
**Phase type:** docs-only. No source / test / script / config / data / manifest / sidecar / gate-report / successor-state mutation. **No** local gitignored output.
...
`e74dc13021900a54153cba81eaed8fdb397fb292` (Phase 4bm-M merge-closeout SHA-finalization commit; pre-branch `main == origin/main` verified in sync).
...
- `docs/00-meta/implementation-reports/2026-05-18_phase-4bm-n_multi-day-v002-label-schema-finalization-memo.md` — the main 43-section schema-finalization memo.
- `docs/00-meta/implementation-reports/2026-05-18_phase-4bm-n_closeout.md` — this closeout.

Modified narrowly:

- `docs/00-meta/current-project-state.md` — new Phase 4bm-N narrative paragraph + new "Current phase:" block; the prior Phase 4bm-M "Current phase:" block is preserved as labelled historical context.
...
## 5. Final schema result

Phase 4bm-N locks the exact v002 label schema at memo level only:

- **Label family identity (final)**: `dataset_family = "microstructure_labels_aggtrades_v001"`, `dataset_version = "v002"`, `label_schema_version = "v001"`, source feature / normalized / raw lineage at `v002`, symbol scope `["BTCUSDT"]`, date range `2024-12-01 .. 2025-02-28` inclusive (90 contiguous UTC days).
- **Column count (final)**: 40 columns per per-day label parquet (17 lineage / identity / metadata + 1 `label_config_hash` + 4 regression labels + 4 classification labels + 12 per-horizon support + 2 global support).
- **Horizons (final)**: `["1s", "5s", "15s", "60s"]` exactly (mirror of Phase 4bj-B v001).
- **Label classes (final)**: 4 × `forward_log_return_<horizon>` (nullable float64) + 4 × `forward_direction_<horizon>` (nullable int8 in `{-1, 0, 1, null}`).
- **Forward-return formula (final)**: `forward_log_return_H = ln(reference_trade_price_H / anchor_trade_price)` (natural log; Decimal-as-string parsed exactly, cast to float64 only at the log step).
- **Direction policy (final)**: strict sign threshold at `0.0` log-return; no dead-band; values `{-1, 0, 1, null}`.
- **v002-specific multi-day adaptations vs Phase 4bj-B v001**:
  - lineage SHAs point to v002 multi-day artefacts (Phase 4bm-L successor-state, Phase 4bm-J gate report, v002 feature / normalized / raw manifests);
  - `source_normalized_manifest_sha256` replaces v001's per-day `source_normalized_parquet_sha256`;
  - `source_raw_manifest_sha256` is added as a new required lineage column;
  - end-of-sample censoring at the 90-day envelope's terminal boundary (`envelope_terminal_unix_ms`, which is the maximum `source_transact_time_ms` across all 90 days; corresponds to 2025-02-28 23:59:59.999 UTC) rather than per-day censoring;
  - cross-day horizon lookahead is allowed inside the label kernel only, within the 90-day v002 envelope.
- **Output namespace (final; not created)**: per-day label parquets at `data/microstructure/labels/microstructure_labels_aggtrades_v001__v002/BTCUSDT/<YYYY>/<MM>/BTCUSDT-labels-aggtrades-<YYYY>-<MM>-<DD>.parquet`; v002 label manifest at `data/microstructure/manifests/microstructure_labels_aggtrades_v001__v002.json`; paired canonical Phase 4bb-F sidecars throughout.
- **Manifest defaults (final)**: `research_eligible = false`, `eligibility_gate_status = "pending"`, `label_family_research_use_authorized = false`, `chronological_split_policy = "not_yet_defined"`, all `governance_labels.*` at their default `forbidden` / `unauthorized` / `allowed_by_future_phase_only` values.

**No label artefact is created, computed, validated, or committed by Phase 4bm-N.** The decision is schema-finalization only.

## 6. Key evidence

| # | Evidence item | Value |
| - | ------------- | ----- |
|  1 | Phase 4bm-L successor-state JSON SHA256 | `7eccaa8f5d9dda9baab08da6914a449e163dda55619d9d734d119d378b5435e4` (recomputed on disk; MATCH) |
...
|  6 | Phase 4bm-J gate report SHA256 | `3c59dfaeb06c14cd1fdf4b589e17ecaa457277637d221bf35c3a69f42a898242` (recomputed on disk; MATCH) |
...
| 10 | v002 feature manifest SHA256 | `512a0a54be40d9c3a61fe0a032ce0301b7b60de60e3147362fa3a4bce633343d` (recomputed on disk; MATCH; unchanged) |
...
| 19 | v002 derived multi-day index manifest SHA256 | `01c5fa538aaa732249483dfac5302945b67461b151cae573cf1cd200e1a2554a` (recomputed on disk; MATCH; unchanged) |
...
| 29 | v001 precedent (label-family schema finalization) | Phase 4bj-B (`docs/00-meta/implementation-reports/2026-05-10_phase-4bj-b_label-schema-finalization.md`); selected Outcome 1 (label schema finalized, implementation deferred); 39-column v001 schema; horizons `{1s, 5s, 15s, 60s}` |
...
- `git log --oneline -12 --decorate`: latest main commit `e74dc13 docs(phase-4bm-m): finalize merge closeout shas` (matches expectation).
- `git diff --check`: clean (exit 0).
- `git diff --name-only`: exactly 3 tracked docs paths (the schema-finalization memo, this closeout, and the narrow `current-project-state.md` update).
...
- The remaining upstream artefacts (per the Phase 4bm-N memo §11 evidence table) were not re-hashed in this phase because Phase 4bm-N reads no Parquet, runs no kernel, and modifies no `data/microstructure/` file; their SHAs are taken verbatim from the Phase 4bm-L successor-state JSON and the Phase 4bm-M merge-closeout.
...
- `ruff check`: **skipped.** Phase 4bm-N modifies no Python source, tests, scripts, or configs. Nothing under `src/prometheus/`, `tests/`, or `scripts/` is touched.
...
Phase 4bm-N is branch-complete only by this work. Per the Phase 4bk-A workflow standard, Phase 4bm-N is **NOT** project-complete until a separately authorized merge phase records its merge-closeout on `main` per the full Tier 1 16-section structure. The operator's broader pause decision continues to apply.

## 11. Explicit non-authorization statement

**Phase 4bm-O / multi-day v002 label-kernel implementation / multi-day v002 label artefact generation / multi-day v002 label artefact structural QA / multi-day v002 label-family eligibility gate / multi-day v002 label-family research-use decision / multi-day v002 label-family successor-state recording / multi-day v002 chronological-split-policy memo / multi-day v002 chronological-split-policy successor-state recording / multi-day v002 diagnostics / multi-day v002 ML / multi-day v002 strategy / multi-day v002 backtests / additional acquisition / Phase 4bm-N merge phase / Phase 4bn-* / Phase 4bo-* / Phase 4bp-* / Phase 4bq-* / Phase 5 / Phase 4 canonical / paper-shadow / live-readiness / deployment / exchange-write / production-key creation / authenticated APIs / private endpoints / user-stream / live WebSocket implementation / public-endpoint calls in code / MCP / Graphify / `.mcp.json` / credentials / any modification of `research_eligible` / `eligibility_gate_status` / `chronological_split_policy` / `stage_4_feature_cleared` on any actual on-disk manifest / any committed `data/microstructure/` artefact / any further successor-state JSON creation / any successor phase whatsoever — all remain unauthorized after Phase 4bm-N.**

## 12. Required exact phrases (verbatim, per task brief)
