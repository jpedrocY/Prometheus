...
**Phase 4bn-F is now merge-complete on main.** **Phase 4bn-F is a docs-only
/ design-only / scoping-only v002 data-sufficiency / representativeness
scoping memo.** **Phase 4bn-F does not acquire data.** **Phase 4bn-F does
not run diagnostics.** **Phase 4bn-F does not run ML.** **Phase 4bn-F does
not train models.** **Phase 4bn-F does not score models.** **Phase 4bn-F
does not generate predictions.** **Phase 4bn-F does not inspect the test
holdout.** **Phase 4bn-F does not use the sealed test split.** **Phase
4bn-F does not rank features.** **Phase 4bn-F does not select features.**
**Phase 4bn-F does not prune features.** **Phase 4bn-F does not engineer
features.** **Phase 4bn-F does not tune hyperparameters.** **Phase 4bn-F
does not tune thresholds.** **Phase 4bn-F does not fit calibrators.**
**Phase 4bn-F does not run strategy research.** **Phase 4bn-F does not
define a strategy.** **Phase 4bn-F does not generate trade signals.**
**Phase 4bn-F does not simulate PnL.** **Phase 4bn-F does not run
backtests.** **Phase 4bn-F does not authorize acquisition.** **Phase 4bn-F
does not authorize storage migration.** **Phase 4bn-F does not create a
v003 dataset.** **Phase 4bn-F does not create a database.** **Phase 4bn-F
does not compact Parquet.** **Phase 4bn-F does not modify dataset layout.**
**Phase 4bn-F does not call any public, authenticated, or private endpoint.**
**Phase 4bn-F does not open any WebSocket or user stream.** **Phase 4bn-F
does not use credentials, `.env`, `.mcp.json`, MCP, or Graphify.** **Phase
4bn-F does not mutate any manifest.** **Phase 4bn-F does not mutate any
...
- **Phase:** Phase 4bn-F — V002 Data-Sufficiency / Representativeness
  Scoping Memo.
- **Type:** docs-only / design-only / scoping-only governance memo. Tier 1
  — Full Phase per `docs/00-meta/process/phase-risk-tiering-standard.md`
  §3, because Phase 4bn-F is adjacent to ML-baseline downstream
  admissibility, possible future data acquisition, possible future v003 /
  longer-history planning, possible future storage architecture decisions,
  and possible future regime / outlier interpretation, while explicitly
  authorizing none of them. The separately authorized scoping phase that
  addresses the operator's standing future data-sufficiency /
  representativeness concern that Phase 4bn-E recorded as a non-authorizing
  note.
- **Action:** merge into `main`.
- **Merge purpose:** bring the Phase 4bn-F scoping memo, closeout, and
  narrow `current-project-state.md` Phase 4bn-F paragraph + new
  Current-phase block onto `main`, recording the decision
...
- **Target branch:** `main`.
- **Source branch:** `phase-4bn-f/v002-data-sufficiency-representativeness-scoping`.

## 2. SHAs

- **`main` SHA before merge:** `8fa219c83326c79ffb6406cc1904440fdc63c376`
  (`docs(phase-4bn-e): finalize merge closeout shas`; `main == origin/main`
  verified pre-merge).
- **Base SHA:** `8fa219c83326c79ffb6406cc1904440fdc63c376`.
- **Branch tip SHA before merge:** `85a74a535ec4532c45415fa2f6ddc416d58d04da`.
- **Branch (docs) commit SHA:** `85a74a535ec4532c45415fa2f6ddc416d58d04da`
  (`docs(phase-4bn-f): scope v002 data sufficiency`; the scoping memo +
  closeout + narrow current-project-state.md paragraph + new Current-phase
...
- Merge commit message: `docs(phase-4bn-f): merge v002 data sufficiency
  scoping`.
...
- `docs/00-meta/implementation-reports/2026-05-29_phase-4bn-f_v002-data-sufficiency-representativeness-scoping.md`
  (added; the phase scoping memo; 20 sections; 1320 lines).
- `docs/00-meta/implementation-reports/2026-05-29_phase-4bn-f_closeout.md`
  (added; the phase closeout; 264 lines).
- `docs/00-meta/current-project-state.md`
  (modified — narrow Phase 4bn-F paragraph + new Current-phase block
  inserted immediately after the Phase 4bn-E paragraph and immediately
  before the existing Phase 4bn-E Current-phase block; prior Phase 4bn-A /
  4bn-B / 4bn-C / 4bn-D / 4bn-E paragraphs and prior Current-phase blocks
  preserved as labelled historical context; +257 / −0).
...
```text
 docs/00-meta/current-project-state.md              |  257 ++++
 .../2026-05-29_phase-4bn-f_closeout.md             |  264 ++++
 ...-data-sufficiency-representativeness-scoping.md | 1320 ++++++++++++++++++++
...
modification to `current-project-state.md` (Phase 4bn-F paragraph + new
...
Phase 4bn-F is the separately authorized docs-only / design-only /
scoping-only governance memo that addresses the operator's standing future
data-sufficiency / representativeness concern about the 90-day v002
microstructure ML-baseline envelope and the standing future storage-
architecture concern about Parquet / DuckDB / database tradeoffs. The memo
evaluates what the current 3-month v002 evidence can support and what it
cannot, defines a representativeness / outlier-risk framework at design
level only, enumerates seven candidate future data-expansion options at
design level only, discusses storage-scaling questions at design level
only, and records a single recommendation that any future work combine
the data-expansion requirements and storage-scaling architecture questions
into a single docs-only / design-only / scoping-only memo because
longer-history microstructure acquisition and storage layout are tightly
coupled. **The recommended successor is recommended only and is not
authorized.** **The operator may equivalently remain paused, reject the
successor and close the ML arc, separately authorize only a data-expansion
requirements memo, or separately authorize only a storage-scaling
architecture memo.** It preserves the Phase 4bn-E decision verbatim
...
modifies no dataset layout, and authorizes no execution. **Phase 4bn-F is
a docs-only / design-only / scoping-only memo and authorizes nothing.**
The v002 label and feature manifests remain `research_eligible = false` /
...
SHA-finalization commit are pushed, Phase 4bn-F is project-complete on
`main`. **Project completion still requires the SHA-finalization commit
below per the repo's current Phase 4bn-E / 4bn-D / 4bn-C / 4bn-B / 4bn-A
SHA-finalization convention.**

### 6.1 Phase 4bn-E decision carried forward
...
executed the bounded descriptive C-D candidate (train-vs-validation feature
drift diagnostics) and produced descriptive feature-drift evidence
(31 low / 13 moderate / 0 high / 1 undefined drift classification; highest
absolute standardized mean delta 0.330; highest absolute missing-rate
delta ≈ 6e-06). Phase 4bn-E partially ruled out gross feature-distribution
drift at the measurement-frame level only; it did not address regime /
volatility / cost-commensurability / calendar-coverage / outlier questions.
Phase 4bn-F inherits this boundary without softening it.

### 6.2 Phase 4bn-D scoping decision carried forward
...
level only. Phase 4bn-F does not revisit the six bounded ML-baseline
expansion candidates; it addresses a different but adjacent standing
future concern (data sufficiency / representativeness / storage scaling)
that Phase 4bn-D / 4bn-E recorded only as a non-authorizing note.

### 6.3 Phase 4bn-C interpretation carried forward

`RECOMMEND_AUTHORIZE_BOUNDED_ML_BASELINE_EXPANSION_SCOPING` (Phase
4bn-C; merge-complete, SHA-finalized, project-complete on `main`). Phase
4bn-F preserves the corrected interpretation of Phase 4bn-B evidence
...
### 6.4 Phase 4bn-B decision carried forward
...
strategy / signal / PnL / backtest exists. Phase 4bn-F inherits every
Phase 4bn-B boundary verbatim.

### 6.5 Phase 4bn-F is a docs-only / design-only / scoping-only memo

Phase 4bn-F adds two new tracked docs files under
...
### 6.6 Core interpretation preserved verbatim
...
- The 90-day v002 window is useful as a first controlled descriptive
  envelope.
- The 90-day v002 window does not prove broad sufficiency.
- The 90-day v002 window does not prove insufficiency.
- The 90-day v002 window does not prove representativeness.
- The 90-day v002 window does not prove it is an outlier.
- "Outlier" remains an unresolved risk, not a conclusion.
- None of the Phase 4bn-A through Phase 4bn-F evidence establishes edge,
...
- Any future acquisition must be preceded by separately authorized
  requirements / scoping work.
- Any future storage change must be preceded by separately authorized
  storage-scaling architecture work.

### 6.7 Recommended successor (NOT authorized)

**Conditional next, NOT authorized.** Phase 4bn-F recommends a future
docs-only / design-only / scoping-only combined data-expansion requirements
+ storage-scaling architecture memo (provisionally Phase 4bn-G; *not*
authorized) as the cleanest non-paused option because longer-history
microstructure acquisition and storage layout are tightly coupled. The
...
- **separately authorize a future docs-only data-expansion requirements
  memo only** (weaker variant of the recommended combined option);
- **separately authorize a future docs-only storage-scaling architecture
  memo only** (weaker variant of the recommended combined option).

Phase 4bn-F does **not** recommend acquisition directly. Phase 4bn-F does
**not** recommend storage migration directly. Phase 4bn-F does **not**
...
under `data/microstructure/`. Phase 4bn-F never opened, read, or hashed
any of them.

## 8. Validation results

- `git diff --check main..phase-4bn-f/v002-data-sufficiency-representativeness-scoping`
  → clean (no whitespace errors).
- `git diff --name-status main..phase-4bn-f/v002-data-sufficiency-representativeness-scoping`
  (pre-merge): `M docs/00-meta/current-project-state.md`;
  `A docs/00-meta/implementation-reports/2026-05-29_phase-4bn-f_closeout.md`;
  `A docs/00-meta/implementation-reports/2026-05-29_phase-4bn-f_v002-data-sufficiency-representativeness-scoping.md`
  (docs only).
- `git diff --stat main..phase-4bn-f/v002-data-sufficiency-representativeness-scoping`
  (pre-merge): `3 files changed, 1841 insertions(+)`.
...
- `git log --oneline -5 --decorate` post-merge confirmed:
  `615c172 (HEAD -> main) docs(phase-4bn-f): merge v002 data sufficiency scoping`
  above
  `85a74a5 (phase-4bn-f/...) docs(phase-4bn-f): scope v002 data sufficiency`
  above
  `8fa219c (origin/main) docs(phase-4bn-e): finalize merge closeout shas`.
...
- Encoding / line-ending preservation:
  ...
  unchanged). No `.gitattributes` amendment was issued by this phase.

## 9. Upstream immutability evidence

Phase 4bn-F is a docs-only / design-only / scoping-only memo that opens
no local data artefact and reads only committed repository Markdown
documents (the Phase 4bn-A through Phase 4bn-E reports and the data /
architecture documents enumerated in the Phase 4bn-F memo §2). It
...
successor-state artefact for mutation. The Phase 4bn-F merge brings forward
zero changes to any prior governed artefact:
...
Phase 4bn-F opened no manifest, no sidecar, no parquet, no CSV, no JSON,
...
All preserved verbatim. All prior phase results (Phase 4am .. Phase 4bn-E)
preserved verbatim.
...
The Phase 4bn-F merge does not, and cannot, be construed as authorizing:
...
- any public / authenticated / private endpoint call; any WebSocket /
  ...
  use;
- Phase 4 canonical; Phase 5; Phase 4bn-G; any further Phase 4bn-*
...
- any future docs-only successor memo (combined data-expansion +
  storage-scaling scoping memo, data-expansion requirements memo only,
  storage-scaling architecture memo only, regime-conditioning,
  class-weighting design, label / target rework, calibration design)
  beyond a separately authorized phase.

`RECOMMEND_AUTHORIZE_COMBINED_DATA_EXPANSION_AND_STORAGE_SCOPING_MEMO__SUBJECT_TO_SEPARATE_OPERATOR_AUTHORIZATION`
means only that Phase 4bn-F records the data-sufficiency /
representativeness scoping decision and recommends remain-paused with a
conditional combined docs-only / design-only / scoping-only successor that
must be separately authorized. **Any future docs-only successor memo
requires a separately authorized phase.** **Any acquisition phase requires
a separately authorized phase.** **Any storage-migration phase requires a
...
- Phase 4bn-G — any future docs-only / design-only / scoping-only combined
  data-expansion requirements + storage-scaling architecture memo (or any
  phase under any name performing the combined memo; or the separately-
  recommended data-expansion-only or storage-scaling-only memos; or any
  acquisition phase; or any storage-migration phase; or any database-
  creation phase; or any Parquet-compaction phase; or any v003-creation
  phase);
...
Phase 4bn-F is now merge-complete on main and, after the SHA-finalization
commit and push, project-complete. The decision
`RECOMMEND_AUTHORIZE_COMBINED_DATA_EXPANSION_AND_STORAGE_SCOPING_MEMO__SUBJECT_TO_SEPARATE_OPERATOR_AUTHORIZATION`
authorizes nothing. **Any future docs-only successor memo requires a
separately authorized phase.** **Phase 4bn-G is not authorized by Phase
4bn-F.** **Recommended state remains paused.**

**Conditional next, NOT authorized:** a Phase 4bn-G docs-only /
design-only / scoping-only combined data-expansion requirements +
storage-scaling architecture memo is the cleanest non-paused option. It
would, if separately authorized later, enumerate the data-sufficiency
dimensions with per-dimension explicit decisions and discuss the
storage-scaling questions at design level only, deciding neither whether
to acquire nor whether to migrate storage, and recording an explicit
non-authorization for both. Phase 4bn-G is **not authorized** by this
merge. The operator may equivalently choose:
...
- **reject further ML-baseline successors and close the ML arc**
  ...
  expansion under current evidence);
- **separately authorize a future docs-only data-expansion requirements
  memo only** (requires a separate authorization prompt; must remain
  docs-only; must not authorize any acquisition);
- **separately authorize a future docs-only storage-scaling architecture
  memo only** (requires a separate authorization prompt; must remain
  docs-only; must not authorize any storage migration or acquisition).
