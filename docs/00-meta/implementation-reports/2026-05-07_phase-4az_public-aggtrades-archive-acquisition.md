...
No code under `src/prometheus/` was modified beyond the new acquisition script; no existing test was modified except a narrow update to one Phase 4ax regression check whose premise (no microstructure data ever) was specific to Phase 4ax. No retained verdict, project lock, M0 governance, or `.gitignore` rule was changed.
