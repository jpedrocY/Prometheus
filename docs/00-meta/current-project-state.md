...
```text
Phase 4f merged into main (External Strategy Research Landscape and V2 Hypothesis Candidate Memo, docs-only).
Phase 4f surveyed external systematic-trading evidence and predeclared V2 — Participation-Confirmed Trend Continuation as a new ex-ante hypothesis candidate.
V2 is pre-research only: not implemented; not backtested; not validated; not live-ready; not a rescue of R3 / R2 / F1 / D1-A.
V2 core premise: trade trend-continuation / breakout events on BTCUSDT perpetual only when price structure, volatility regime, participation / volume, and derivatives-flow context align.
V2 timeframe matrix: signal candidates 15m / 30m / 1h; bias candidates 1h / 4h; session / volume bucket candidates 30m / 1h; 5m diagnostic-only, not primary signal.
V2 bounded feature policy: max 8 active entry features; max 3 active exit / regime features; all thresholds must be predeclared before any backtest.
Whole-repo quality gates remain clean: ruff check . passed; pytest 785 passed; mypy strict 0 issues across 82 source files.
Phase 4f recommendation: Option B (docs-only V2 strategy-spec memo) primary; Option C (docs-only V2 data-requirements and feasibility memo) conditional secondary; Option A (remain paused) procedurally acceptable; Option D (immediate data acquisition) NOT recommended; Option E (immediate exploratory backtest) REJECTED; Option F (paper/shadow / live-readiness / exchange-write) FORBIDDEN.
No implementation code changed in Phase 4f.
No data was acquired by Phase 4f.
No backtests were run by Phase 4f.
...
Phase 4g / any successor phase remains unauthorized.
V2 strategy-spec memo, V2 data-requirements / feasibility memo, V2 implementation, V2 backtests, V2 data acquisition all remain unauthorized.
...
```text
main HEAD:                          4535f463a9759716fe3665b551f9a2d949d4a62e
Merge title:                        Merge Phase 4f (external strategy research landscape and V2 hypothesis candidate memo, docs-only) into main
Phase 4f research-memo commit:      4f5d12b97e0127daaef6bea7e96db322fb98d053
Phase 4f closeout commit:           4a7cc9f515af9d0669932d57acff4b304a005b4c
Phase 4f merge commit:              4535f463a9759716fe3665b551f9a2d949d4a62e
```
