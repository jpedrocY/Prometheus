...
```text
Phase 4j merged into main (V2 Metrics Data Governance Memo, docs-only).
Phase 4j adopts the Phase 4j §11 metrics OI-subset partial-eligibility rule as binding governance for the Phase 4i metrics partial-pass evidence.
The rule mirrors Phase 3r §8 mark-price gap governance, transposed from per-trade exclusion to per-bar exclusion.
Metrics manifests remain globally research_eligible: false. Phase 4i metrics manifests are NOT modified. No corrected metrics manifests are created. No __v002 metrics manifests are created. No v003 is created.
Feature-level partial-eligibility is allowed only for the OI subset: create_time; symbol; sum_open_interest; sum_open_interest_value.
Optional ratio columns remain feature_eligible: false and forbidden for V2 first backtest: count_toptrader_long_short_ratio; sum_toptrader_long_short_ratio; count_long_short_ratio; sum_taker_long_short_vol_ratio.
Per-bar exclusion rule: any 30m V2 signal bar is OI-feature-eligible only if all six aligned 5-minute metrics records are present and non-NaN in OI columns; any 30m signal bar failing this check is excluded from V2 candidate setup generation.
No forward-fill, interpolation, imputation, replacement, synthetic OI data, or silent omission is allowed.
Future V2 backtest must report exclusion counts and the required sensitivity analysis: main-cell V2 results vs. exclude-entire-affected-days sensitivity cell.
OI delta computation rule: last completed 5-minute OI of current 30m window vs. last completed 5-minute OI of previous 30m window.
The rule is immutable absent a separately authorized governance amendment.
Immediate V2 backtest remains unauthorized.
Future V2 backtest-plan memo (future Phase 4k) is conditional secondary only and was NOT started by the Phase 4j merge.
V2 implementation remains unauthorized.
Optional ratio-column activation remains unauthorized.
OI feature removal from V2 remains unauthorized.
Mark-price 30m / 4h acquisition remains unauthorized.
aggTrades acquisition remains unauthorized.
...
No data acquired. No data modified. No manifests modified. No source code modified. No tests modified. No backtests run.
No retained verdicts were revised.
No project locks changed.
Phase 4 (canonical) remains unauthorized.
Phase 4k / any successor phase remains unauthorized.
...
Phase 4j §11 metrics OI-subset partial-eligibility rule is binding from this merge forward; immutable absent a separately authorized governance amendment.
...
```text
main HEAD:                          ea948017c77f4a028427f9c8515e973cd274abad
Merge title:                        Merge Phase 4j (V2 metrics data governance memo, docs-only) into main
Phase 4j memo commit:               6fa92766da5b47ef1d5ebf2b7f7c8334e15b317e
Phase 4j closeout commit:           1b2d4c8f57bd4356e67b8a35a255fbf8d5d9ce5e
Phase 4j merge commit:              ea948017c77f4a028427f9c8515e973cd274abad
```
