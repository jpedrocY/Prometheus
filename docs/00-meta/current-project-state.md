...
```text
Phase 4b merged into main (Repository Quality Gate Restoration: 29 known pre-existing Ruff issues in scripts/phase3q_5m_acquisition.py and scripts/phase3s_5m_diagnostics.py fixed via behavior-preserving lint-only edits).
ruff check scripts: passed.
pytest: 785 passed (no regressions).
mypy strict: 0 issues across 82 source files.
ruff check . (whole repo): 2 residual pre-existing errors in Phase 4a state code (state/__init__.py:20:1 I001 import order; state/transitions.py:49:5 SIM103 simplify return). Documented and not fixed by Phase 4b; require separate operator authorization if fixed later.
Phase 4 (canonical) remains unauthorized.
Phase 4c / any successor phase remains unauthorized.
...
```text
main HEAD:                          f099a946165a905bfbb4cb7129144b5abdda6a12
Merge title:                        Merge Phase 4b (repository quality gate restoration: scripts ruff cleanup) into main
Phase 4b implementation commit:     d375ffb0e4725e0bd759eb56e18aad5d9ac94c36
Phase 4b closeout commit:           1c6d36bfbb0bd869325b4cd773a1d25584bdbcce
Phase 4b merge commit:              f099a946165a905bfbb4cb7129144b5abdda6a12
```
