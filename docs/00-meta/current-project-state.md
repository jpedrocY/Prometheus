...
```text
Phase 4g merged into main (V2 Strategy-Spec Memo: Participation-Confirmed Trend Continuation, docs-only).
Phase 4g operationalized V2 — Participation-Confirmed Trend Continuation into a bounded predeclared strategy spec.
V2 remains pre-research only: not implemented; not backtested; not validated; not live-ready; not a rescue of R3 / R2 / F1 / D1-A.
Selected timeframe matrix: signal 30m; HTF bias 4h (EMA(20)/(50) discrete comparison); session / volume bucket 1h; 5m diagnostic-only, not primary signal.
Selected feature set: 8 active entry features (HTF trend bias state; Donchian breakout state; Donchian width percentile; range-expansion ratio; relative volume + volume z-score; volume percentile by UTC hour; taker buy/sell imbalance; OI delta direction + funding-rate percentile band) + 3 active exit / regime features (time-since-entry counter; ATR percentile regime; HTF bias state continuity).
Threshold grid: 512 predeclared combinatorial variants over 9 non-fixed binary axes (N1, P_w max, V_rel_min, V_z_min, T_imb_min, OI_dir, funding band, N_R, T_stop); future backtest phase MUST apply PBO / deflated Sharpe / CSCV treatment per Bailey/Borwein/Lopez de Prado/Zhu 2014 or reduce the search space before testing.
Predeclared M1 / M2 / M3 mechanism-check decomposition committed before any V2 backtest exists.
Governance labels: stop_trigger_domain research = trade_price_backtest; stop_trigger_domain future runtime / paper / live = mark_price_runtime; stop_trigger_domain future mark-price validation = mark_price_backtest_candidate; break_even_rule = disabled; ema_slope_method = discrete_comparison; stagnation_window_role = metric_only; mixed_or_unknown invalid and fails closed for all four schemes.
Whole-repo quality gates remain clean: ruff check . passed; pytest 785 passed; mypy strict 0 issues across 82 source files.
Phase 4g recommendation: Phase 4h V2 Data Requirements and Feasibility Memo (docs-only) primary; remain paused conditional secondary; immediate data acquisition NOT recommended; immediate backtest REJECTED; V2 implementation REJECTED; paper/shadow / live-readiness / exchange-write FORBIDDEN.
No implementation code changed in Phase 4g.
No data acquired by Phase 4g.
No backtests run by Phase 4g.
No strategy was implemented or validated.
No retained verdicts were revised.
No project locks changed.
Phase 4 (canonical) remains unauthorized.
Phase 4h / any successor phase remains unauthorized.
V2 data-requirements / feasibility memo, V2 implementation, V2 backtests, V2 data acquisition all remain unauthorized.
...
```text
main HEAD:                          ce9659f61f5ec145e3cadd59305bd923bac73061
Merge title:                        Merge Phase 4g (V2 strategy-spec memo: Participation-Confirmed Trend Continuation, docs-only) into main
Phase 4g spec commit:               f6265cecfb7120c6459a70862fee69ac26bf5d30
Phase 4g closeout commit:           194075070961fa883523c49baa775469c05e8e96
Phase 4g merge commit:              ce9659f61f5ec145e3cadd59305bd923bac73061
```
