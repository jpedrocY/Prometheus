...
```text
Phase 4l merged into main (V2 Backtest Execution, docs-and-code).
Phase 4l implemented and ran the standalone V2 backtest exactly under the Phase 4k methodology and emitted Verdict C — V2 framework HARD REJECT.
Phase 4l added: scripts/phase4l_v2_backtest.py (standalone research script); docs/00-meta/implementation-reports/2026-04-30_phase-4l_v2-backtest-execution.md; docs/00-meta/implementation-reports/2026-04-30_phase-4l_closeout.md.
Local Phase 4l research outputs under data/research/phase4l/ are gitignored and not committed; reproducible from the orchestrator script with pinned RNG seed 202604300.
Final Verdict: C — V2 framework HARD REJECT.
Binding catastrophic-floor driver: CFP-1 critical: 512 / 512 variants produced fewer than 30 OOS trades on BTCUSDT; BTC-train-best variant produced 0 OOS trades.
CFP-3 mechanically triggered (PF=0 under empty arrays) but subordinate to CFP-1.
Root cause: V2's locked 20/40-bar Donchian setup window per Phase 4g §29 axis 1 is structurally incompatible with the V1-inherited 0.60-1.80 × ATR(20) stop-distance filter per Phase 4g §19 / V1 §Stop-distance filter; raw V2 candidates produce stop distances around 3-5 × ATR(20), so all are rejected before trade generation.
M1 / M2 / M3 mechanism checks NOT meaningfully evaluable due to 0-trade populations.
BTCUSDT primary result: all 512 variants × 3 cost cells × 3 windows produced trade_count=0, mean_R=0, total_R=0, sharpe=0.
ETHUSDT comparison result: identical structural 0-trade pattern; ETH does not rescue BTC.
Phase 4j §11 metrics OI-subset governance honored exactly; per-bar exclusion algorithm matches Phase 4j §16 verbatim.
Metrics OI exclusion fraction: 33 / 74 448 30m bars excluded per symbol (~0.044%); CFP-9 not triggered.
Optional ratio-column access: zero (static scan + metrics-loader explicit column list + runtime introspection).
Forbidden data access: zero (no mark-price 30m/4h/5m/15m, no aggTrades, no spot, no cross-venue, no authenticated/private/public-endpoint-in-code, no user stream, no WebSocket, no listenKey, no credentials, no network I/O).
PBO / DSR / CSCV reported but methodologically inert under 0-trade population: PBO=0.500; DSR=0 for all variants; CSCV S=16 with exact 12 870 combinations honored.
§11.6 HIGH cost preserved verbatim at 8 bps slippage per side; mechanically mean_R=0 but methodologically irrelevant under 0-trade root cause.
V2 first-spec is terminally HARD REJECTED as retained research evidence only.
V2 remains pre-research only: not implemented; not validated; not live-ready; not a rescue of R3 / R2 / F1 / D1-A.
No V2 rescue authorized: no V2-prime, V2-narrow, V2-relaxed, V2 hybrid, stop-distance amendment, N1 amendment, Phase 4g amendment, Phase 4j §11 amendment, Phase 4k methodology amendment.
No V2 implementation occurred.
No src/prometheus code modified.
No tests modified.
...
Phase 4m / any successor phase remains unauthorized.
...
```text
main HEAD:                          918b10a60df23481c25522cdeb51941abecfd44e
...
Phase 4l merge commit:              918b10a60df23481c25522cdeb51941abecfd44e
```
