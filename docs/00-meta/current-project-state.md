...
```text
Phase 4h merged into main (V2 Data Requirements and Feasibility Memo, docs-only).
Phase 4h translated the locked Phase 4g V2 strategy spec into exact data requirements and feasibility rules.
...
Minimum future V2 first-backtest acquisition set: BTCUSDT 30m trade-price klines; ETHUSDT 30m trade-price klines; BTCUSDT 4h trade-price klines; ETHUSDT 4h trade-price klines; BTCUSDT metrics; ETHUSDT metrics.
Funding-rate history reuses existing v002 funding manifests.
Mark-price 30m / 4h is deferred to a future mark_price_backtest_candidate path.
aggTrades is optional / deferred fallback only.
Phase 4h predeclared: dataset-versioning convention; directory-layout proposal; manifest requirements; integrity-check requirements; research_eligible rules; invalid-window handling; timestamp / alignment policy; public-source feasibility; forbidden / private-data boundary.
Whole-repo quality gates remain clean: ruff check . passed; pytest 785 passed; mypy strict 0 issues across 82 source files.
Phase 4h recommendation: Phase 4i V2 Public Data Acquisition and Integrity Validation (docs-and-data, public bulk archives only, no credentials) primary; remain paused conditional secondary; revise V2 spec NOT recommended; immediate V2 backtest REJECTED; V2 implementation REJECTED; paper/shadow / live-readiness / exchange-write FORBIDDEN.
No implementation code changed in Phase 4h.
No data acquired.
No data downloaded.
...
Phase 4i / any successor phase remains unauthorized.
V2 data acquisition, V2 implementation, V2 backtests all remain unauthorized.
...
```text
main HEAD:                          99aacbc80760405a49ea19ff848642ebb0960cba
Merge title:                        Merge Phase 4h (V2 data requirements and feasibility memo, docs-only) into main
Phase 4h memo commit:               6ad42080bf91e82eef3113ab9166c15f86ccdd3f
Phase 4h closeout commit:           a27ebdd671446b2be563d84adcce27878894c73b
Phase 4h merge commit:              99aacbc80760405a49ea19ff848642ebb0960cba
```
