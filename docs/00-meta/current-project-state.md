...
```text
Phase 4i merged into main (V2 Public Data Acquisition and Integrity Validation, docs-and-data).
Phase 4i acquired the six Phase 4h-predeclared minimum V2 dataset families from public unauthenticated data.binance.vision bulk archives (BTC + ETH 30m klines, BTC + ETH 4h klines, BTC + ETH metrics).
Phase 4i did NOT acquire mark-price 30m / 4h (DEFERRED).
Phase 4i did NOT acquire aggTrades (OPTIONAL / DEFERRED).
Phase 4i did NOT re-acquire funding-rate (v002 reused).
Phase 4i added scripts/phase4i_v2_acquisition.py (standalone, no credentials).
Phase 4i generated 6 new manifests under data/manifests/.
Phase 4i verdict: PARTIAL PASS.
Research-eligible datasets: binance_usdm_btcusdt_30m__v001; binance_usdm_ethusdt_30m__v001; binance_usdm_btcusdt_4h__v001; binance_usdm_ethusdt_4h__v001.
Not research-eligible datasets: binance_usdm_btcusdt_metrics__v001; binance_usdm_ethusdt_metrics__v001.
Metrics failure reason: intra-day 5-minute missing observations (BTC 5,699 / ETH 3,631 missing) + NaN values in optional ratio columns concentrated in early-2022; required sum_open_interest and sum_open_interest_value columns FULLY POPULATED (zero NaN) for both symbols.
Phase 4i did NOT relax strict gate rules.
Phase 4i did NOT patch, forward-fill, interpolate, or silently omit data.
Phase 4i stops for operator review.
V2 remains pre-research only: not implemented; not backtested; not validated; not live-ready; not a rescue of R3 / R2 / F1 / D1-A.
Immediate V2 backtest remains REJECTED.
V2 implementation remains REJECTED.
A future docs-only metrics governance memo analogous to Phase 3r §8 is conditional secondary only and was NOT started by the Phase 4i merge.
...
Phase 4j / any successor phase remains unauthorized.
...
```text
main HEAD:                          a2c414e8cf7e36235db4c97905f9cc890a848e70
Merge title:                        Merge Phase 4i (V2 public data acquisition and integrity validation, docs-and-data) into main
Phase 4i acquisition commit:        6913d29ff70c683e2f10b61c873241adaffecfcf
Phase 4i closeout commit:           408301f1eb5b16cfac35f73936738bd973b42caa
Phase 4i merge commit:              a2c414e8cf7e36235db4c97905f9cc890a848e70
```
