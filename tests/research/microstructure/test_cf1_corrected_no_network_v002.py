"""Phase 4bn-BB — static no-network / no-credentials / no-reserve guard for corrected CF-1 code.

Scans the three corrected CF-1 source modules and the BB orchestration script for forbidden
imports (networking / exchange / dotenv) and forbidden credential / MCP / ``.env`` / endpoint
tokens, and asserts the runtime governance / non-authorization flags are all false. Also asserts
the prohibited mean column is absent from the requested source-column list.
"""
...
def test_prohibited_mean_not_in_source_columns() -> None:
