"""Phase 4bm-J static no-network / no-credential / no-MCP scan.

Scans the four new Phase 4bm-J source modules + the Phase 4bm-J
...
PHASE_4BM_J_MODULES: tuple[str, ...] = (
    "multiday_feature_gate_io.py",
    "multiday_feature_gate_report.py",
    "multiday_feature_gate_checks.py",
    "multiday_feature_gate.py",
)

PHASE_4BM_J_SCRIPTS: tuple[str, ...] = (
    "phase4bm_j_run_multiday_feature_gate.py",
)
...
def _phase_4bm_j_files() -> list[Path]:
    files = [PACKAGE_ROOT / name for name in PHASE_4BM_J_MODULES]
    files += [SCRIPTS_ROOT / name for name in PHASE_4BM_J_SCRIPTS]
    return files


@pytest.mark.parametrize("module_path", _phase_4bm_j_files(), ids=lambda p: p.name)
def test_phase_4bm_j_module_has_no_forbidden_imports(module_path: Path) -> None:
...
_DENY_TOKENS: tuple[str, ...] = (
    ...
    ".env", "Graphify", "MCP", ".mcp.json",
)


def test_phase_4bm_j_modules_have_no_credential_tokens_in_code() -> None:
    for module_path in _phase_4bm_j_files():
        text = module_path.read_text(encoding="utf-8")
...
def test_phase_4bm_j_module_files_exist() -> None:
    for module_path in _phase_4bm_j_files():
