"""Phase 4bm-Q static no-network / no-credential / no-MCP scan.

Scans the four new Phase 4bm-Q source modules + the Phase 4bm-Q
orchestrator script for forbidden imports, credential-shaped tokens,
and MCP / Graphify references in code (excluding docstrings /
comments).
"""
...
PHASE_4BM_Q_MODULES: tuple[str, ...] = (
    "multiday_label_gate_io.py",
    "multiday_label_gate_report.py",
    "multiday_label_gate_checks.py",
    "multiday_label_gate.py",
)

PHASE_4BM_Q_SCRIPTS: tuple[str, ...] = (
    "phase4bm_q_run_multiday_label_gate.py",
)
...
def _phase_4bm_q_files() -> list[Path]:
    files = [PACKAGE_ROOT / name for name in PHASE_4BM_Q_MODULES]
    files += [SCRIPTS_ROOT / name for name in PHASE_4BM_Q_SCRIPTS]
    return files


@pytest.mark.parametrize("module_path", _phase_4bm_q_files(), ids=lambda p: p.name)
def test_phase_4bm_q_module_has_no_forbidden_imports(module_path: Path) -> None:
...
# with legitimate identifiers like ``envelope_terminal_unix_ms`` used by the
# Phase 4bm-N label schema. The boundary requires that ``.env`` is followed
# by a non-word character or end-of-string (so ``.env`` and ``.env.local``
# trip the check but ``envelope_*`` does not).
_DOT_ENV_RE = re.compile(r"\.env(?![A-Za-z0-9_])")


def test_phase_4bm_q_modules_have_no_credential_tokens_in_code() -> None:
    for module_path in _phase_4bm_q_files():
        text = module_path.read_text(encoding="utf-8")
...
def test_phase_4bm_q_module_files_exist() -> None:
    for module_path in _phase_4bm_q_files():
