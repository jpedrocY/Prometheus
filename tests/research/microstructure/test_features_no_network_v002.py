"""Phase 4bm-H v002 static no-network / no-credential / no-MCP scan.

Scans the four new Phase 4bm-H v002 source modules plus the Phase 4bm-H
orchestrator script for forbidden imports, credential-shaped tokens,
and MCP / Graphify references in code (not docstrings).
"""
...
PHASE_4BM_H_MODULES: tuple[str, ...] = (
    "features_schema_v002.py",
    "features_io_v002.py",
    "features_compute_v002.py",
    "features_manifest_v002.py",
)

PHASE_4BM_H_SCRIPTS: tuple[str, ...] = (
    "phase4bm_h_compute_multiday_features.py",
)
...
def _phase_4bm_h_files() -> list[Path]:
    files = [PACKAGE_ROOT / name for name in PHASE_4BM_H_MODULES]
    files += [SCRIPTS_ROOT / name for name in PHASE_4BM_H_SCRIPTS]
    return files


@pytest.mark.parametrize(
    "module_path", _phase_4bm_h_files(), ids=lambda p: p.name
)
def test_phase_4bm_h_module_has_no_forbidden_imports(module_path: Path) -> None:
...
def test_phase_4bm_h_modules_have_no_credential_tokens_in_code() -> None:
    for module_path in _phase_4bm_h_files():
        text = module_path.read_text(encoding="utf-8")
...
def test_phase_4bm_h_module_files_exist() -> None:
    for module_path in _phase_4bm_h_files():
