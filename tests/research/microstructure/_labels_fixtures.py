"""Phase 4bj-C label test fixtures: tiny synthetic feature + normalized parquets.

These helpers build self-consistent, very small pyarrow tables that
match the Phase 4bd-/Phase 4bh-shaped contracts well enough for label
unit tests (anchor policy, future-reference policy, censoring, sign,
invalid-price). Tests use pytest ``tmp_path`` only and never read or
...
def build_normalized_table(
    *,
    ...
    The returned table has the 19 Phase 4bd canonical columns in
    ...
    data = {
        ...
        "source_gate_report_id": ["microstructure_raw_aggtrades_v001__v001__fixture"]
        * n,
...
def build_feature_table(*, normalized: pa.Table) -> pa.Table:
    """Build a feature-shaped pyarrow Table aligned 1:1 with *normalized*.

    The feature kernel keeps ``row_index``, ``agg_trade_id``, and
    ``feature_timestamp_ms == source_transact_time_ms == normalized.transact_time_ms``
    aligned per row. Phase 4bj-C only reads four columns from the
    feature table, so we expose just those four (the test schema is
    intentionally minimal to keep fixtures fast and side-effect-free).
    """
    schema = pa.schema(
        [
            pa.field("row_index", pa.int64(), nullable=False),
