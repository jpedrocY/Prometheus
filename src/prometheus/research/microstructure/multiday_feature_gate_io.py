"""Phase 4bm-J multi-day v002 feature-family eligibility gate I/O helpers.

Provides read-only loaders for the Phase 4bm-H v002 feature artefacts
plus path discipline + atomic JSON / sidecar writers under
``data/microstructure/gate-reports/features/`` for the gitignored gate
report output. Mirrors the Phase 4bi-B (v001) and Phase 4bm-D (v002
multi-day derived) IO contracts.
...
- does NOT mutate any source artefact (feature parquet, feature
  manifest, sidecars, normalized parquets, derived manifest, raw
  manifest, gate reports, successor-state JSONs);
- writes only under
  ``data/microstructure/gate-reports/features/`` for the gate report
  JSON + paired Phase 4bb-F sidecar;
...
FEATURE_GATE_REPORTS_SUBDIR_PARTS = ("data", "microstructure", "gate-reports", "features")


class MultidayFeatureGateIOError(RuntimeError):
    """Raised when a Phase 4bm-J IO operation fails closed."""


def assert_path_under_microstructure(path: Path, *, label: str) -> None:
    if not isinstance(path, Path):
        raise MultidayFeatureGateIOError(f"{label} must be a pathlib.Path")
...
def assert_path_under_feature_gate_reports(path: Path, *, label: str) -> None:
    assert_path_under_microstructure(path, label=label)
    if not _resolve_path_parts_under(path, FEATURE_GATE_REPORTS_SUBDIR_PARTS):
        raise MultidayFeatureGateIOError(
            f"{label} must resolve under data/microstructure/gate-reports/features/ "
            f"(got {path!s})"
        )
...
def compute_file_sha256(path: Path) -> tuple[str, int]:
    ...
    if not isinstance(path, Path):
        raise MultidayFeatureGateIOError("path must be a pathlib.Path")
    h = hashlib.sha256()
    size = 0
...
def read_json_file(path: Path) -> tuple[dict[str, Any], str, int]:
    ...
    if not isinstance(path, Path):
        raise MultidayFeatureGateIOError("path must be a pathlib.Path")
    if not path.exists():
        raise MultidayFeatureGateIOError(f"file does not exist: {path}")
    raw = path.read_bytes()
    ...
    if not isinstance(parsed, dict):
        raise MultidayFeatureGateIOError(f"file {path} top-level JSON must be an object")
...
def derive_gate_report_id(
    *,
    ...
    if not isinstance(unix_ms, int) or unix_ms <= 0:
        raise MultidayFeatureGateIOError("unix_ms must be positive int")
    if not short_commit or len(short_commit) < 8:
        raise MultidayFeatureGateIOError("short_commit must be at least 8 hex chars")
    return (
...
def derive_gate_report_paths(
    *,
    ...
    """Return ``(report_json_path, sidecar_path)`` under the features gate-report root."""
    if not isinstance(output_root, Path):
        raise MultidayFeatureGateIOError("output_root must be Path")
    ...
    if "features" not in output_root.parts:
        # Append 'features' if the operator passed gate-reports/ root.
        raise MultidayFeatureGateIOError(
            "output_root must contain a 'features' segment for Phase 4bm-J"
        )
...
def compose_canonical_sidecar_body(*, sha256_hex: str, basename: str) -> bytes:
    ...
    if not isinstance(sha256_hex, str) or len(sha256_hex) != 64 or sha256_hex.lower() != sha256_hex:
        raise MultidayFeatureGateIOError("sha256_hex must be 64-char lowercase hex")
    if not isinstance(basename, str) or not basename or "\n" in basename:
        raise MultidayFeatureGateIOError("basename must be a non-empty single-line string")
...
def atomic_write_json(
    ...
    *,
    ...
    if not isinstance(path, Path):
        raise MultidayFeatureGateIOError("path must be Path")
    assert_path_under_feature_gate_reports(path, label="gate report output")
    if refuse_overwrite and path.exists():
        raise MultidayFeatureGateIOError(f"refusing to overwrite existing file: {path}")
...
__all__ = [
    "FEATURE_GATE_REPORTS_SUBDIR_PARTS",
